...
module MPIServerSubs
    ...
    subroutine MPIServer_Init(C_filename_len, C_filename, nTout, ncout, ierror) BIND(C, NAME='MPIServer_Init')
        ...
        if(verbose) write(output_unit_eff,*) "Server init (dll): Connection established with communicators: ", client_comm, " (client/server)",  MPI_COMM_WORLD,"(global); error:", ierror
    ...
    subroutine MPIServer_Stop(ierror) BIND(C, NAME='MPIServer_Stop')
        ...
        if(textout) close(output_unit_eff)    
        call cleanup()
